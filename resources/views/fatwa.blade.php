...
<div class="container p-4 mx-auto">
    <p class="tracking-out-expand text-center text-xl text-red-700"> এখানে আপনার প্রয়োজনীয় ফতুয়া পড়ূন </p>
    <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
        ...
        <div class="p-6 bg-white rounded-lg shadow-md">
            <div class="flex items-center gap-3">
                {{-- @dd($post[6]) --}}
                @if (!empty($post[5]))
                <img src="{{ Storage::url($post[5]) }}" class="object-cover w-10 h-10 border border-gray-300 rounded-full shadow-sm" alt="Profile Image">
                @else
                ...
                <div class="text-sm text-gray-500">
                    ...
                    <p>Posted: {{ optional(\Carbon\Carbon::parse($post[2] ?? ''))->diffForHumans() ?? 'Unknown time' }}</p>
                </div>
            </div>
